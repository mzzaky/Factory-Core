...
public class FactoryManager {
            ...
            try {
                ...
                if (ownerStr != null && !ownerStr.equals("null")) {
                    try {
                        ...
                        plugin.getLogger().warning(String.format("Invalid owner UUID for factory %s: %s", key, ownerStr));
                    }
                }
    ...
    public Factory createFactory(String id, String regionName, FactoryType type, double price, Location fastTravelLocation) {
    ...
    public boolean buyFactory(Player player, String id) {
        ...
        if (factory.getOwner() != null) return false;
    ...
    public boolean sellFactory(Player player, String id) {
        Factory factory = getFactory(id);
        if (factory == null) return false;
        if (!player.getUniqueId().equals(factory.getOwner())) return false;
    ...
    public void updateProduction() {
        for (Factory factory : factories.values()) {
            ...
            if (factory.getStatus() != FactoryStatus.RUNNING) continue;
            if (factory.getCurrentProduction() == null) {
    ...
    public boolean upgradeFactory(Player player, String id) {
        ...
        if (factory.getLevel() >= maxLevel) return false;
        
        double upgradeCost = factory.getPrice() * 0.5 * factory.getLevel();
        if (!plugin.getEconomy().has(player, upgradeCost)) return false;
