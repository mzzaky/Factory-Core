...
public class FactoryCoreAPI {
    ...
    /**
     * Start a factory upgrade (consumes resources + money, begins upgrade timer).
     * 
     * @param player    Player upgrading the factory
     * @param factoryId Factory ID
     * @return true if upgrade successfully started, false otherwise
     */
    public boolean upgradeFactory(Player player, String factoryId) {
        return plugin.getFactoryManager().startUpgrade(player, factoryId);
    }
 ...
 *           api.getFactoriesByOwner(player.getUniqueId());
 *           getLogger().info(player.getName() + " owns " + factories.size() + "
 *           factories");
 * 
 ...
 *           api.getPlayerInvoices(player.getUniqueId());
 *           getLogger().info(player.getName() + " has " + invoices.size() + "
 *           unpaid invoices");
 *           }
 *           }
 * 
 ...
 *               getLogger().info("Factory " + factory.getId() + " completed
 *               producing " + recipe.getName());
 *               }
 */